...
public static class TRT // v1, top-down, recursion with memoization
{
    ...
    public static int Solve(int[] treatValues)
        => SolveWithMemoization(treatValues, new IndexRange(0, treatValues.Length - 1), 1, new Dictionary<IndexRange, int>());
        ...
        int[] treatValues, IndexRange indexRange, int age, Dictionary<IndexRange, int> memoizer)
    {
        ...
        result = Math.Max(
            age * treatValues[indexRange.Start]
                + SolveWithMemoization(treatValues, new IndexRange(indexRange.Start + 1, indexRange.End), age + 1, memoizer),
            age * treatValues[indexRange.End]
                + SolveWithMemoization(treatValues, new IndexRange(indexRange.Start, indexRange.End - 1), age + 1, memoizer));
