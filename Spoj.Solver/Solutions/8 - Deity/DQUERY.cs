...
public static class DQUERY
{
    ...
    // - You can do that using a BIT/segment tree with same cardinality as the source array, with
    // the standard sum operation.
    // - For the general case, you can read all the input at once (offline algorithm), and sort it
    // in a specific way, then leverage the update operation on the BIT between query phases.

    // If you still need help after thinking about that a lot, consider querying and updating 0s
    // and 1s in the BIT, where 1s signify the latest occurrence (or first, depending on how you
    // traverse & order things) of the value at that index in the source array. How will you order
    // the queries? How/when will you update the BIT (just a small update each time)?
    public static int[] SolveOffline(int[] sourceArray, DistinctCountQuery[] queries)
    {
        ...
        // indices, which is why the query objects were sorted that way below. A PURQ BIT is queried
            ...
            ++phaseEndIndex)
        {
            ...
            {
                queryResults[query.ResultIndex] = purqBinaryIndexedTree.SumQuery(
