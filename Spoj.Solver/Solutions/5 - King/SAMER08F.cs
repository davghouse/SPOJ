...
// https://www.spoj.com/problems/SAMER08F/ #ad-hoc #dynamic-programming-1d #math
...
public static class SAMER08F
{
    ...
    static SAMER08F()
    {
        ...
        for (int n = 3; n <= _limit; ++n)
        {
            ...
            // For example, growing from the top left for n = 3 there's square n = 1,
            // and square n = 2, and this repeats for each corner.
            squareCounts[n] += 4 * (n - 1);

            // Add the squares that touch the parent square's edge but don't touch one of
            // the parent square's four corners. For example, for n = 4 there's two unused
            // edge squares on each side, from the first there's two (n = 1, n = 2) second
            // there's one (n = 1). In general, for each side there's n - 2 squares to start
            // from, the first gives n - 2 squares, the second n - 3 squares, etc, to one.
