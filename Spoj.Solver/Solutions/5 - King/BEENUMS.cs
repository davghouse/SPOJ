...
public static class BEENUMS
{
    ...
    // an integer. That's done in a roundabout way by rounding it to an integer, computing
    // the beehive number from that index, and checking to see if it's equal to "number"...
    public static bool Solve(int number)
    {
        // 4.0 to avoid integer overflow as the input number can be a billion.
        int indexOfClosestBeehiveNumber
            = (int)Math.Round(1 / 6.0 * (Math.Sqrt(3 * (4.0 * number - 1)) + 3));
    ...
    // ...Or more directly by just checking to see if it's equal to its int cast,
