...
public static class STPAR
{
    ...
    {
        // Due to street widths, love mobiles can't go around each other. As each love mobile
        // arrives on the approach street, we can either push it down the side street, or move
        // it into its final parade position. Before deciding where to move it we can also pop
        // any mobiles already on the side street out of it and into their final parade position.
        ...
        for (int i = 0; i < approachStreetLoveMobiles.Length; ++i)
        {
            ...
            if (nextLoveMobileOnTheApproachStreet == nextLoveMobileNeededOnTheParadeStreet)
            {
                ...
                // Once a love mobile is pushed onto the side street, it'll have to come before
                // anything already on the side street in the final parade ordering, since it'll
                // be popped out before them. So its designated ordering better not conflict with
                // what we know has to happen.
        ...
        // necessarily consecutively)--except at the end of the loop. Then the side street must have
        // all the love mobiles not already on the parade street, so it has a contiguous and consecutive
        // range of integers and that are hence pop-able to the parade street in the correct order.
        return true;
    }
}
...
public static class Program
{
    private static void Main()
    {
        ...
        {
            int[] approachStreetLoveMobiles = Array.ConvertAll(
