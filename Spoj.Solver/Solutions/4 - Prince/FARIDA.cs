...
public static class FARIDA
{
    ...
    // monster definitely isn't chosen), and the solution through the first k - 1 monsters.
    // So 1D dynamic programming where the array's index is the solution when travelling
    // only past monsters [0 ... index]. Another way to do it is having the array store
    // the best solution that necessarily includes the monster at the same index. Then I
    // think we'd have to look 3 back and we couldn't read off the answer directly from
    // the final index.
