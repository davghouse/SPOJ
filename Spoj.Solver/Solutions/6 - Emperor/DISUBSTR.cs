...
public static class DISUBSTR // Same problem as SUBST1.
{
    ...
    // in O(n^2logn). I don't actually create the substrings though. We don't need to create the
    // LCP array, but the same concept is used to count distinct substrings.
    public static int Solve(string s)
    {
        ...
        // Every substring is the prefix of a suffix, the suffix starting at the substring's start
        // index, the prefix as long as the substring. Having the suffixes ordered allows us to
        // count distinct substrings by comparing adjacent sorted suffixes. The ith sorted suffix
        // has some prefix length in common with the ith + 1, so those substrings are repeated in
        // the string. We wait to count the ones in common until they become not in common for some
        // j > i in a later iteration. For the length that's not in common with the ith + 1, we count
        // it now. Distinct substrings are only counted once because they're not counted until the
        // first time they're not in common with the next suffix, and once they're not in common with
        // the next suffix, they never happen again, since the suffixes are ordered.
        ...
        for (int i = 0; i < s.Length - 1; ++i)
        {
            ...
            while (prefixLengthInCommon < maxPrefixLengthInCommon
                && s[currentSuffixStartIndex + prefixLengthInCommon]
                == s[nextSuffixStartIndex + prefixLengthInCommon])
            {
        ...
        // Everything in the last suffix still needs to be counted, since anything in common with
